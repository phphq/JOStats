...
//                                                           //
// File Name     : Player Vehicle Stats Module               //
...
if(!$tpl->getMgtTpl($file)) {
	...
	$varArray["formAction"] = "./?";
	$varArray["section"] = "player_vehicles";
	$varArray["pid"] = $pid;
	$varArray["page"] = $page;
    ...
    $cond = $tpl->getjoConditionsS($condServer, $condStats, $condTeam, $condGame, $pid, "vehicles");
	...
	foreach($headArray as $title => $stat) {
		...
		$sortUrl = "./?section=player_vehicles&pid=".$pid."&page=".$page."&sort=".$stat."&order=".$ord."&condStats=".$condStats."&condTeam=".$condTeam."&condServer=".$condServer."&condGame=".$condGame;
		$varArray["headers"] .= '<td title="Sort table by '.GetJoHeaders($title).' '.$ord.'">
		                         <a href="'.$sortUrl.'">'.$title.'</a></td>';
	}
	
	$sql = $tpl->joQuery("SELECT ".$cond["tableV"].".vehicle, ".$cond["table"].".player 
	                      FROM ".$cond["tableV"].", ".$cond["table"]."
						  WHERE ".$cond["tableV"].".record = ".$cond["table"].".id 
	...
	$url = "./?section=player_vehicles&pid=".$pid."&sort=".$sort."&order=".$order."&condStats=".$condStats."&condTeam=".$condTeam."&condServer=".$condServer."&condGame=".$condGame."&page=";

	$varArray["multiList"] = $tpl->GetJoMultiList($url, $page, $total, $perPage);
	
	$varArray["content"] = $tpl->buildMgtTpl($varArray, $sqlArray, 1, $file, "vehicles");
}